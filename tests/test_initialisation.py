...
def test_inventory_has_initial_sections(initial_warehouse_app):
    assert list(initial_warehouse_app.inventory_manager.sections.keys()) == ['Electronics', 'Automotive']

def test_initial_sections_are_empty(initial_warehouse_app):
    for section in initial_warehouse_app.inventory_manager.sections.values():
